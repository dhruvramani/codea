...
class Seq2Seq(torch.nn.Module):
    ...
    """
    def __init__(self, encoder, decoder, config, onnx=False, dense=None, lm_head=None, beam_size=None, max_length=None, sos_id=None,eos_id=None):
        super(Seq2Seq, self).__init__()
        self.encoder = encoder
        self.decoder = decoder
        self.config = config
        self.register_buffer("bias", torch.tril(torch.ones(2048, 2048)))
        self.dense = torch.nn.Linear(config.hidden_size, config.hidden_size) if dense is None else dense
        self.lm_head = torch.nn.Linear(config.hidden_size, config.vocab_size, bias=False) if lm_head is None else lm_head
